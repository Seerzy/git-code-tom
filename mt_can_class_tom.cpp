...
public:
    ...
    static CanMotorController* instance; // Declare as static
    ...
    float getPositionDegrees(){
        return radiansToDegrees(output_position);
    }

    static void handleInterrupt(int signal) {
        if (instance) {
            instance->handleInterruptMember(signal);
        }
    }
...
CanMotorController* CanMotorController::instance = nullptr; // Initialize the static member


int main(){
    CanMotorController mit_controller;
    CanMotorController::instance = &mit_controller; // Set the instance pointer

    while (true){
        mit_controller.setTorque(-1.33);
        float torque = mit_controller.getTorque();
        float position = mit_controller.getPositionDegrees();
